...
import { InitiativeEditForm } from "./initiative-edit-form"

interface EditInitiativeProps {
  params: { locale: string; id: string }
}

export default function EditInitiative({ params }: EditInitiativeProps) {
  ...
  return (
    <div className={`space-y-4 ${fontClass}`}>
        ...
        <div className="flex items-center gap-2 px-4">
          ...
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbLink href={`/${params.locale}/admin/initiatives`} className={fontClass}>
                  Initiatives
                </BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator className="hidden md:block" />
      ...
      <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
        <div className="flex items-center justify-between space-y-2">
          <div>
            ...
            <p className={`text-muted-foreground ${fontClass}`}>
              Update initiative details, manage team and tasks
            </p>
          </div>
          <div className="flex items-center space-x-2">
            <Button variant="outline" asChild className={fontClass}>
              <Link href={`/${params.locale}/admin/initiatives`}>
        ...
        {/* Form */}
        <InitiativeEditForm locale={params.locale} initiativeId={params.id} />
      </div>
    </div>
  )
}
